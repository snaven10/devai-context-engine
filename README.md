...
> Quick reference below. Full guide — config layers & precedence, MCP client wiring, the complete env-var
> table, and git auto-index hooks — in **[docs/11-configuration.md](docs/11-configuration.md)**
> ([español](docs/es/11-configuracion.md)).

### Project Config (`.devai/config.yaml`)

Auto-generated by `devai init`. The schema defines the project name and path, `state_dir`, embedding provider and model, storage mode, and indexing excludes.

> Full config.yaml schema: [docs/11-configuration.md §1.1](docs/11-configuration.md).

### Environment Variables

> These are the common ones. Full authoritative table (30+ vars): [docs/11-configuration.md §3](docs/11-configuration.md).

| Variable | Description | Default |
|----------|-------------|---------|
| `DEVAI_STATE_DIR` | Override state directory | `~/.local/share/devai/state` |
| `DEVAI_EMBEDDING_MODEL` | Embedding model key *(overridden by `config.yaml` — see below)* | `minilm-l6` |
| `DEVAI_LOCAL_DB_PATH` | Override SQLite database path | *(derived from state dir)* |
| `DEVAI_EMBED_MAX_CHARS` | Max characters fed to the embedder per chunk | `2048` |
| `DEVAI_TOKEN_STRATEGY` | Tool-response budget: `drop` / `soft_truncate` / `hard_truncate` / `summarize` | `drop` |
| `DEVAI_MAX_OUTPUT_TOKENS` | Max tokens in a single MCP tool response | `4000` |

> **Precedence:** the model in `.devai/config.yaml` **overrides** `DEVAI_EMBEDDING_MODEL` — the CLI/MCP pass
> the config value to the Python service. Change the model in `config.yaml` (or `devai model use <key>`), not
> just the env var. Full env reference + tuning vars: **[Configuration](docs/11-configuration.md)**.
...
|-----|-------|-----------|-------|
| `minilm-l6` | all-MiniLM-L6-v2 | 384 | Default. Fast, lightweight (English). |
| `minilm-l12` | all-MiniLM-L12-v2 | 384 | Better quality, slightly slower (English). |
| `bge-small` | BAAI/bge-small-en-v1.5 | 384 | Good for English code. |
| `bge-base` | BAAI/bge-base-en-v1.5 | 768 | Best English quality, 2x dimension. |
| `ml-minilm` | paraphrase-multilingual-MiniLM-L12-v2 | 384 | **Multilingual** (50+ langs), fast. |
| `ml-mpnet` | paraphrase-multilingual-mpnet-base-v2 | 768 | **Multilingual**, best quality. |
| `ml-granite` | granite-embedding ONNX int8 | 384 | ONNX int8, ~94 MB — **recommended for CPU multilingual**. |
| `ml-granite-lg` | granite-embedding-lg ONNX int8 | 768 | ONNX int8, ~299 MB. Higher-quality multilingual. |

> Picking a model for your hardware: [docs/09-models-and-tuning.md §1 & §5](docs/09-models-and-tuning.md).

See **[Models & Tuning](docs/09-models-and-tuning.md)** for strengths, hardware-based picks, and the
...
- [Configuration](docs/11-configuration.md) — `config.yaml`, MCP client wiring, env vars, git auto-index hooks ([español](docs/es/11-configuracion.md))
